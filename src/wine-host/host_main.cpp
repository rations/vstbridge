...
public:
    ...
    bool connect(const std::string& path) {
        ...
        if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
            LOG_ERROR("WineSocketClient: WSAStartup failed");
            return false;
        }

        socket_ = ::socket(AF_UNIX, SOCK_STREAM, 0);
        if (socket_ == INVALID_SOCKET) {
            LOG_ERROR("WineSocketClient: socket() failed: {}", WSAGetLastError());
            WSACleanup();
            return false;
        }

        if (path.length() >= sizeof(sockaddr_un::sun_path)) {
            LOG_ERROR("WineSocketClient: socket path too long");
            close();
            return false;
        }
        ...
        {
            LOG_ERROR("WineSocketClient: connect() failed: {}", WSAGetLastError());
            close();
            return false;
        }
                     ...
                     size_t payloadSize) noexcept
    {
        ...
        // timestamp: not critical on Wine side, set to 0
        header.timestamp    = 0;

        if (!sendAll(&header, sizeof(header))) return false;
        if (payloadSize > 0 && payload) {
            if (!sendAll(payload, payloadSize)) return false;
        }
        return true;
    }
    ...
    bool receiveMessage(GenericMessage& msg, int timeoutMs = 0) noexcept {
        ...
        if (!recvAll(&msg.header, sizeof(msg.header))) return false;

        if (msg.header.magic != MessageHeader::kMagic) {
            LOG_ERROR("WineSocketClient: bad magic 0x{:x}", msg.header.magic);
            return false;
        }
        ...
        if (msg.header.payload_size > 0) {
            ...
            if (!recvAll(msg.payload.data(), msg.header.payload_size)) return false;
        }
