...
static constexpr const char* YABRIDGE_HOST_EXPECTED_PREFIX = "Usage: yabridge-";
...
bool verify_path_setup() {
    // First check ~/.local/share/yabridge directly (yabridge always searches there)
    try {
        auto xdg_home = yabridge_data_home();
        auto host_exe = xdg_home / YABRIDGE_HOST_EXE_NAME;
    ...
    if (shell == "ash" || shell == "bash" || shell == "csh" || shell == "ksh" ||
        ...
        args = {"-l", "-c",
                std::string("command -v ") + YABRIDGE_HOST_EXE_NAME};
    } else if (shell == "elvish" || shell == "oil") {
        args = {"-c", std::string("command -v ") + YABRIDGE_HOST_EXE_NAME};
    } else if (shell == "pwsh") {
        args = {"-l", "-c", std::string("which ") + YABRIDGE_HOST_EXE_NAME};
    } else if (shell == "nu") {
        args = {"-c", std::string("which ") + YABRIDGE_HOST_EXE_NAME};
    } else {
        std::cerr << "\n"
                  << wrap_text("WARNING: Yabridgectl does not know how to handle your login shell "
                               ...
                               "a feature request in order to get yabridgectl to support your "
    ...
    std::cerr << "\n"
              << wrap_text("Warning: '" + std::string(YABRIDGE_HOST_EXE_NAME) +
                           "' is not present in either '~/.local/share/yabridge' your login "
                           "shell's search path. You may not have extracted yabridge's files to "
...
void verify_wine_setup(Config& config) {
    ...
    // Unset WAYLAND_DISPLAY like yabridge itself does
    ...
    auto exe_so = files.yabridge_host_exe_so ? files.yabridge_host_exe_so
                                             : files.yabridge_host_32_exe_so;
    if (!exe_so)
        throw std::runtime_error("Could not locate '" + std::string(YABRIDGE_HOST_EXE_NAME) +
                                 ".so'");
    ...
    if (config.last_known_config && config.last_known_config->wine_version == current.wine_version &&
        config.last_known_config->yabridge_host_hash == current.yabridge_host_hash)
        return;

    // Pick the right host binary based on Wine prefix architecture
    fs::path host_path;
    if (get_default_wine_prefix_arch() == LibArchitecture::Lib32) {
        if (!files.yabridge_host_32_exe)
            throw std::runtime_error("Could not find '" +
                                     std::string(YABRIDGE_HOST_32_EXE_NAME) + "'");
        host_path = *files.yabridge_host_32_exe;
    } else {
        if (!files.yabridge_host_exe)
            throw std::runtime_error("Could not find '" + std::string(YABRIDGE_HOST_EXE_NAME) +
                                     "'");
        host_path = *files.yabridge_host_exe;
    }
    ...
    while (std::getline(iss, line)) {
        if (line.substr(0, std::strlen(YABRIDGE_HOST_EXPECTED_PREFIX)) ==
            YABRIDGE_HOST_EXPECTED_PREFIX) {
            success = true;
            break;
        }
        // Ignore fixme lines
    ...
    if (success) {
        ...
        std::cerr << "\n"
                  << wrap_text(
                         "Warning: Could not run '" + std::string(YABRIDGE_HOST_EXE_NAME) +
                         "'. Wine reported the following error:\n\n" +
                         (last_error.empty() ? "<no output>" : last_error) +
                         "\n\nMake sure that you have downloaded the correct version of yabridge "
                         "for your distro.\nThis can also happen when using a version of Wine "
                         "that's not compatible with this version of yabridge, in which case "
                         ...
                         "yabridge#troubleshooting-common-issues")
                  << "\n";
    }
}
...
void verify_external_dependencies() {
    ...
    std::cerr << "\n"
              << wrap_text(std::string("Warning: Could not find '") + LIBDBUS_NAME +
                           "'. This will not prevent yabridge from working, but you will also not "
