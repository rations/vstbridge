...
fs::path yabridge_data_home() {
    return xdg_data_home() / "yabridge";
}

fs::path yabridgectl_config_dir() {
    return xdg_config_home() / "yabridgectl";
}
...
fs::path yabridge_vst2_home() {
    ...
    if (!home)
        throw std::runtime_error("$HOME is not set");
    return fs::path(home) / ".vst" / "yabridge";
}
...
fs::path yabridge_vst3_home() {
    ...
    if (!home)
        throw std::runtime_error("$HOME is not set");
    return fs::path(home) / ".vst3" / "yabridge";
}
...
fs::path yabridge_clap_home() {
    ...
    if (clap_path_env && clap_path_env[0] != '\0') {
        ...
        return fs::path(first) / "yabridge";
    }
    const char* home = std::getenv("HOME");
    if (!home)
        throw std::runtime_error("$HOME is not set");
    return fs::path(home) / ".clap" / "yabridge";
}
...
static fs::path config_file_path() {
    return yabridgectl_config_dir() / "config.toml";
}
...
Config Config::read() {
    ...
    if (auto v = tbl["yabridge_home"].value<std::string>())
        c.yabridge_home = fs::path(*v);
    ...
    if (auto* sub = tbl["last_known_config"].as_table()) {
        ...
        kc.yabridge_host_hash = sub->at("yabridge_host_hash").value_or(int64_t{0});
...
void Config::write() const {
    fs::path dir = yabridgectl_config_dir();
    std::error_code ec;
    ...
    if (yabridge_home)
        tbl.insert_or_assign("yabridge_home", yabridge_home->string());
    ...
    if (last_known_config) {
        ...
        sub.insert_or_assign("yabridge_host_hash", last_known_config->yabridge_host_hash);
...
YabridgeFiles Config::files() const {
    auto data_home = yabridge_data_home();
        ...
        if (!found) {
            ...
            if (fs::exists(candidate)) {
                ...
                throw std::runtime_error(
                    ...
                    "'. You can override the default search path using 'yabridgectl set "
                    "--path=<path>'.");
            }
        }
    }
    ...
    auto host_exe = which(YABRIDGE_HOST_EXE_NAME);
    ...
    auto host_32_exe = which(YABRIDGE_HOST_32_EXE_NAME);
