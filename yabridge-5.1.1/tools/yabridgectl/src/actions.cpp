...
void show_status(const Config& config) {
    ...
    std::cout << "yabridge path: "
              << (config.yabridge_home
                      ? ("'" + config.yabridge_home->string() + "'")
                      : std::string("<auto>"))
              << "\n";

    if (config.vst2_location == Vst2InstallationLocation::Centralized)
        std::cout << "VST2 location: '" << yabridge_vst2_home().string() << "'\n";
    else
        std::cout << "VST2 location: inline next to the Windows plugin file\n";

    std::cout << "VST3 location: '" << yabridge_vst3_home().string() << "'\n";
    std::cout << "CLAP location: '" << yabridge_clap_home().string() << "'\n\n";

    const YabridgeFiles* files_ptr = nullptr;
    std::optional<YabridgeFiles> files_storage;
    try {
        ...
        std::cout << YABRIDGE_HOST_EXE_NAME << ": ";
        if (f.yabridge_host_exe && f.yabridge_host_exe_so)
            std::cout << "'" << f.yabridge_host_exe->string() << "'\n";
        else
            std::cout << color::red("<not found>") << "\n";

        std::cout << YABRIDGE_HOST_32_EXE_NAME << ": ";
        if (f.yabridge_host_32_exe && f.yabridge_host_32_exe_so)
            std::cout << "'" << f.yabridge_host_32_exe->string() << "'\n";
        else
            std::cout << color::red("<not found>") << "\n";
    } catch (const std::exception& e) {
        std::cout << "Could not find yabridge's files: " << e.what() << "\n";
    }
...
void set_settings(Config& config, const SetOptions& opts) {
    if (opts.path)
        config.yabridge_home = opts.path;
    if (opts.path_auto)
        config.yabridge_home = std::nullopt;
...
void do_sync(Config& config, const SyncOptions& opts) {
    ...
    auto vst2_home = yabridge_vst2_home();
    auto vst3_home = yabridge_vst3_home();
    auto clap_home = yabridge_clap_home();
    ...
    bail_if_unsafe(vst2_home, "~/.vst/yabridge");
    bail_if_unsafe(vst3_home, "~/.vst3/yabridge");
    bail_if_unsafe(clap_home, "~/.clap/yabridge");
