...
fs::path Vst2Plugin::centralized_native_target() const {
    ...
    if (subdirectory)
        return yabridge_vst2_home() / *subdirectory / file_name;
    return yabridge_vst2_home() / file_name;
}
...
fs::path Vst3Module::target_bundle_home() const {
    ...
    if (subdirectory)
        return yabridge_vst3_home() / *subdirectory / name;
    return yabridge_vst3_home() / name;
}

fs::path Vst3Module::target_native_module_path(const YabridgeFiles* files) const {
...
fs::path ClapPlugin::native_target() const {
    ...
    if (subdirectory)
        return yabridge_clap_home() / *subdirectory / file_name;
    return yabridge_clap_home() / file_name;
}
...
SearchResults::installation_status(const Config& config, const YabridgeFiles* files) const {
