...
int main(int argc, char** argv) {
    // Append ~/.local/share/yabridge to PATH so yabridge-host.exe can be found
    try {
    ...
    CLI::App app{"yabridgectl - Manage yabridge plugin installations", "yabridgectl"};
    ...
    // ── sync ─────────────────────────────────────────────────────────────────
    auto* sync_cmd = app.add_subcommand("sync", "Set up or update yabridge for all plugins");
    ...
    // ── set ──────────────────────────────────────────────────────────────────
    auto* set_cmd = app.add_subcommand("set", "Change the yabridge path (advanced)");
    ...
    set_cmd->add_option("--path", set_path_str,
                        "Path to the directory containing 'libyabridge-chainloader-{clap,vst2,vst3}.so'")
        ->check([](const std::string& s) { return validate_directory(s); });
    set_cmd->add_flag("--path-auto", set_path_auto, "Automatically locate yabridge's files");
